...
public class B_GroupByAndPartition {
	...
	public static void main(String[] args) {
		...
		List<Empleado> empleados = Arrays.asList(new Empleado("Pepe", 30000, "Ventas"),
				new Empleado("Juan", 31000, "Ventas"), new Empleado("Antonio", 30000, "Compras"),
				new Empleado("María", 30000, "Compras"), new Empleado("Manuela", 34000, "Ejecutivo"));
...
//		// Podemos particionar en dos grupos, según un predicado
		Map<Boolean, List<Empleado>> salarioMayorOIgualque32000 = empleados.stream()
				.collect(partitioningBy(e -> e.getSalario() >= 32000));// recibe como argumento un predicado en el que
																		// comprobamos si el sueldo va a ser mayor o
																		// igual a 32000

		System.out.println("EMPLEADOS AGRUPADOS POR EL CUMPLIMIENTO DE UNA FUNCIÓN");
		salarioMayorOIgualque32000.forEach((bool, v) -> {
			System.out.printf("Empleados con un salario " + ((bool) ? "mayor" : "menor") + " que 32000€: %s %n",
					v.stream().map(e -> e.getNombre()).collect(joining(", ")));
